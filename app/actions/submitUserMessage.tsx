...
export async function submitUserMessage(input: string): Promise<ClientMessage> {
  ...
  const result = await streamUI({
    ...
    tools: {
      ...
      searchPlaces: {
        description:
          'Search suggestions of places on base a given destination and category(activities, hotels, clubs,  restaurants)',
        parameters: z.object({
          destination: z
            .string()
            .describe(
              'The location where the user wants the suggestions of the places',
            ),
          category: z
            .string()
            .describe('Number of people who are planned to go on the trip'),
